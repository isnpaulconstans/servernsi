...
if [ "$1" == "on" ] ; then
    ...
    sed -i -e "s/\/\/\$this->pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION)/\$this->pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION)/" /var/www/Projet_ISN/src/models/Database.php
else
